...
export const env = createEnv({
    server: {
        ...
        SERVER_CONNECTION_TIMEOUT_MS: z.coerce.number().int().min(1).default(120_000),
        TRUST_PROXY_HOPS: z.coerce.number().int().min(0).default(0),
        FILE_PURGE_RETENTION_DAYS: z.coerce.number().int().min(1).default(30),
        UPLOAD_CONCURRENCY_LIMIT: z.coerce.number().int().min(1).default(8),
        THUMBNAIL_CONCURRENCY_LIMIT: z.coerce.number().int().min(1).default(4),
