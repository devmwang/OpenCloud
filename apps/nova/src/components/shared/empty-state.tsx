...
export function EmptyState({ icon, title, description, action }: EmptyStateProps) {
    return (
        <div className="flex flex-col items-center justify-center px-4 py-14">
            <div className="border-border mb-4 flex h-16 w-16 items-center justify-center rounded-xl border-2 border-dashed">
                {icon ?? <FolderIcon className="text-text-dim h-8 w-8" />}
            </div>
            <h3 className="text-text mb-1 text-base font-medium">{title}</h3>
            {description ? <p className="text-text-muted max-w-sm text-center text-sm">{description}</p> : null}
            {action ? (
