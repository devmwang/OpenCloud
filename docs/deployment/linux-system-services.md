...
This fast-forwards to the latest code from Git (using the repo path saved at install time), reloads the updated service script, runs `pnpm install --frozen-lockfile`, builds, and restarts the service(s). For `server` and `both`, it creates a persistent server-start block, stops the server, applies migrations, and removes the block only after migration succeeds. If a migration fails, the server remains blocked and stopped across reboots until a successful retry. Update and rebuild preserve the existing boot enablement policy. Add a mode to limit the work to one app: `update server` or `update nova`.
